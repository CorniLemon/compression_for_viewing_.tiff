...
public:
    ...
    TiffFile(FILE* f) {//читает header
        ...
        if (!height || !width || !bitOnPix||!startOfPtrToStripes||!sizeof(startsStrips)||!stringsInStripe||!countOfChanals) {//проверка наличи€ некотрых тегов 
        ...
        auto print = [&]() {//вывод в [], т.к. эти данные не будут хранитьс€ в пам€ти после закрыти€ конструктора
    ...
    vector<WORD> getLine(FILE* f, size_t i) {//получает номер строки и читает еЄ. fread вызываетс€
